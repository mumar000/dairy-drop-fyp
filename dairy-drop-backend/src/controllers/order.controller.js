...
export const placeOrder = asyncHandler(async (req, res) => {
  ...
  for (const item of orderItems) {
    const p = storeProducts.find(
      (pp) => String(pp._id) === String(item.product),
    );
    if (!p)
      return res
        .status(404)
    ...
    if (!p.isActive)
      return res.status(400).json({ message: `Product inactive: ${p.name}` });
...
export const requestRefund = asyncHandler(async (req, res) => {
  ...
  if (order.paymentMethod !== "Stripe") {
    return res
      .status(400)
      .json({
        message: "Refund requests are only available for Stripe orders",
      });
  }
  ...
  if (order.paymentStatus !== "Paid") {
    return res
      .status(400)
      .json({ message: "Only paid orders can request a refund" });
  }
  ...
  try {
    ...
    // Socket server may not be available in serverless/prod
    console.log(error);
  }
