...
const Orders = () => {
  ...
  const [markCheckoutSessionFailed] = useMarkCheckoutSessionFailedMutation();
  ...
  const paymentStatusConfig = {
    ...
    RefundRequested: "bg-amber-100 text-amber-700 border-amber-200",
    RefundPending: "bg-orange-100 text-orange-700 border-orange-200",
    Refunded: "bg-emerald-100 text-emerald-700 border-emerald-200",
    RefundRejected: "bg-rose-100 text-rose-700 border-rose-200",
    RefundFailed: "bg-red-100 text-red-700 border-red-200",
  };
  ...
  const tabs = [
    ...
    },
    {
      ...
      count: orders.filter(
        (o) =>
          o.status === "Confirmed" ||
          ["RefundRequested", "RefundPending"].includes(o.paymentStatus),
      ).length,
    },
    {
      id: "Pending",
    ...
    const matchesTab =
      ...
      (activeTab === "Processing" &&
        (order.status === "Confirmed" ||
          ["RefundRequested", "RefundPending"].includes(order.paymentStatus))) ||
  ...
  const handleRequestRefund = async () => {
    try {
      ...
      toast.error(error?.data?.message || "Failed to submit refund request");
    }
  };
  ...
  useEffect(() => {
    ...
    if (!stripeStatus || !sessionId) return;

    let isMounted = true;

    const syncStripeResult = async () => {
      try {
        if (stripeStatus === "success") {
          ...
          await markCheckoutSessionFailed(sessionId).unwrap();
          if (!isMounted) return;
        ...
        if (!isMounted) return;
        toast.error(
          error?.data?.message || "Unable to sync Stripe payment status",
        );
      } finally {
        if (!isMounted) return;
                ...
                >
                  ...
                  <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border ${statusConfig[order.status]?.color || "bg-gray-100 text-gray-700 border-gray-200"}`}
                        >
                          <StatusIcon
                            className={`w-4 h-4 ${statusConfig[order.status]?.iconColor || "text-gray-600"}`}
                          />
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-6 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-gray-600">Payment:</span>
                            <span
                              className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${paymentStatusConfig[order.paymentStatus] || "bg-gray-100 text-gray-700 border-gray-200"}`}
                            >
                              {order.paymentStatus}
                            </span>
                          </div>
                          ...
                          {order.paymentMethod === "Stripe" &&
                            order.paymentStatus === "Paid" && (
                              <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition text-sm font-medium"
                              >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6">
              ...
              <p className="text-gray-600 mb-4">
                Select the reason for your refund request. The admin will review
                it before any Stripe refund is issued.
              </p>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={isRequestingRefund}
                  className="px-4 py-2 bg-amber-600 text-white hover:bg-amber-700 rounded-lg transition font-medium disabled:opacity-50 flex items-center gap-2"
                >
                  {isRequestingRefund ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
